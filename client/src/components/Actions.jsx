...
const Actions = ({post:post_}) => {
  ...
  const handleLikeAndUnlike = async() => {
    if (!user) return showToast("Error", "Please Log in first to like a post ", "error")
    if (isLiking) return ; 
    setIsLiking(true);
    try {
  ...
  const handleReply = async () => {
    if (!user) return showToast("Error", "You must be logged in to replay to a post", "error")
  ...
  return (
    ...
    <Flex gap={3} my={2} onClick={(e)=> e.preventDefault()}>
      ...
      >
        <ModalOverlay />
        <ModalContent>
          {/* <ModalHeader>Reply your account</ModalHeader> */}
          <ModalCloseButton />
          <ModalBody pb={6}>
            <FormControl>
              ...
              <Input  placeholder='Reply goes here..'
                value={replay}
                onChange={(e) => setRepaly(e.target.value)}
              />
            </FormControl>
          </ModalBody>
